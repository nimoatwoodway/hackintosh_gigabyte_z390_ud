 ...
 *
 * Disassembly of SSDT-6-A M I.aml, Tue Mar 19 07:29:01 2019
 *
...
DefinitionBlock ("", "SSDT", 2, "ALASKA", "A M I", 0x00000000)
{
    ...
    External (UMP3, UnknownObj)    // (from opcode)
    ...
    If (LGreaterEqual (NHSP, 0x0B))
    {
        Scope (\_SB.PCI0.XHC.RHUB.HS11)
        {
            Method (_UPC, 0, NotSerialized)  // _UPC: USB Port Capabilities
            {
                If (LEqual (HBTC, Zero))
                {
                    If (LEqual (And (UMAP, 0x0400), 0x0400))
                    {
                        Return (GUPC (One))
                    }
                    Else
    ...
    If (LGreaterEqual (NHSP, 0x0C))
    {
        Scope (\_SB.PCI0.XHC.RHUB.HS12)
        {
            Method (_UPC, 0, NotSerialized)  // _UPC: USB Port Capabilities
            {
                If (LEqual (HCTC, Zero))
                {
                    If (LEqual (And (UMAP, 0x0800), 0x0800))
                    {
                        Return (GUPC (One))
                    }
                    Else
    ...
    If (LGreaterEqual (NHSP, 0x0D))
    {
        Scope (\_SB.PCI0.XHC.RHUB.HS13)
        {
            Method (_UPC, 0, NotSerialized)  // _UPC: USB Port Capabilities
            {
                If (LEqual (HDTC, Zero))
                {
                    If (LEqual (And (UMAP, 0x1000), 0x1000))
                    {
                        Return (GUPC (One))
                    }
                    Else
    ...
    If (LGreaterEqual (NHSP, 0x0E))
    {
        Scope (\_SB.PCI0.XHC.RHUB.HS14)
        {
            Method (_UPC, 0, NotSerialized)  // _UPC: USB Port Capabilities
            {
                If (LEqual (HETC, Zero))
                {
                    If (LEqual (And (UMAP, 0x2000), 0x2000))
                    {
                        Return (GUPC (One))
                    }
                    Else
    ...
    If (LGreaterEqual (NSSP, 0x09))
    {
        Scope (\_SB.PCI0.XHC.RHUB.SS09)
        {
            Method (_UPC, 0, NotSerialized)  // _UPC: USB Port Capabilities
            {
                If (LEqual (S9TC, Zero))
                {
                    If (LEqual (And (UMP3, 0x0100), 0x0100))
                    {
                        Return (GUPC (One))
                    }
                    Else
    ...
    If (LGreaterEqual (NSSP, 0x0A))
    {
        Scope (\_SB.PCI0.XHC.RHUB.SS10)
        {
            Method (_UPC, 0, NotSerialized)  // _UPC: USB Port Capabilities
            {
                If (LEqual (SATC, Zero))
                {
                    If (LEqual (And (UMP3, 0x0200), 0x0200))
                    {
                        Return (GUPC (One))
                    }
                    Else
